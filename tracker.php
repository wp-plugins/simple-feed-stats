...
if (isset($_GET["sfs_tracking"])) {
	...
	$host = 'n/a'; $req = 'n/a'; $referer = 'n/a'; $qstring = 'n/a'; $address = 'n/a'; $agent = 'n/a';

	if (isset($_SERVER["HTTP_HOST"]))       $host    = sfs_cleaner($_SERVER["HTTP_HOST"]);
	if (isset($_SERVER["REQUEST_URI"]))     $req     = sfs_cleaner($_SERVER["REQUEST_URI"]);
	...
	if ($options['sfs_tracking_method'] == 'sfs_custom_tracking') {
		...
		$feed_type = sfs_cleaner($_GET["feed_type"]);
		$tracking = 'custom';

		if     ($feed_type == 'rdf')      $type = 'RDF';
		elseif ($feed_type == 'rss')      $type = 'RSS';
		elseif ($feed_type == 'feed')     $type = 'RSS2';
	...
	if (($options['sfs_tracking_method'] == 'sfs_alt_tracking')) {
		...
		if     ($referer == $feed_rdf)       $type = 'RDF';
		elseif ($referer == $feed_rss)       $type = 'RSS';
		elseif ($referer == $feed_rss2)      $type = 'RSS2';
		elseif ($referer == $feed_atom)      $type = 'Atom';
		elseif ($referer == $feed_coms)      $type = 'Comments';
		elseif ($referer == $feed_coms_atom) $type = 'Comments';
		elseif ($referer == $feed_coms_rdf)  $type = 'Comments';
		else                                 $type = 'other';
	...
	if (($options['sfs_tracking_method'] == 'sfs_open_tracking')) {
		...
		$feed_type = sfs_cleaner($_GET["sfs_type"]);
		$tracking = 'open';
		$type = 'custom';
